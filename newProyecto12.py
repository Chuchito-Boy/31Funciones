...
def Movies10(tupla:tuple):
    list_aux = [(i[0],i[3]) for i in tupla if i[6]=="C"]
    def porcentual()->tuple:
        my_list = map(lambda x: x[0]+','+str(int(float(x[1])*10))+'%' ,list_aux)
        return tuple(my_list)
    return porcentual
